<template>
  <div>
    ...
    >
      ...
      <div v-if="filterable" class="common-tree-head">
...
export default {
  ...
  methods: {
    ...
    setSelectOption(node) {
      if (node) {
        let tmpMap = {};
        tmpMap.value = node.data[this.nodeKey];
        tmpMap.label = node.label ? node.label : node.data?.label;
        this.options = [];
        this.options.push(tmpMap);
        this.selectedData = node.data[this.nodeKey];
        if (this.valueKey) {
          this.selectedDataMap = {
            value: node.data[this.nodeKey],
            label: node.data[this.valueKey],
          };
        }
      }
    },
    ...
    handleNodeClick(data, node) {
      if (!this.multiple) {
        ...
        if (this.valueKey) {
          this.$emit("change", this.selectedDataMap);
        } else {
          this.$emit("change", this.selectedData);
        }
      }
